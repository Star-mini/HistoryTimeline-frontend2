...
import ContentsPopup from "../popup/contentP/ContentsPopup";
...
const Timeline = () => {
  ...
  ); // scroll의 y 위치에 따른 타임라인의 길이
  const [selectedCountry, setSelectedCountry] = useState(
    countryId === undefined || Number(countryId) === 410
      ? null
      : countries.find((country) => country.countryId === Number(countryId))
  ); // 선택된 나라
  const [selectedYear, setSelectedYear] = useState({ name: 1500 }); // 선택된 연도 - 가져온 연도 리스트에서 첫번째 연도로 했음.
  const countriesExcludeKorea = countries.filter(
    (country) => country.countryId !== 410
  );
  ...
  const handleMovieSelect = (movieTitle) => {
    ...
    setIsMovieSelected(true); // 영화 선택 시 isMovieSelected 상태 업데이트
  };
  ...
  const Modal = ({
    ...
    const handleClose = () => {
      setIsMovieSelected(false); // 모달 닫을 때 상태 초기화
      onClose(); // 모달 닫기 함수 호출
    };
  ...
  return (
    <div>
      ...
      <Modal
        ...
        onClose={() => {
          setIsModalOpen(false);
          setIsMovieSelected(false); // 모달 닫을 때 영화 선택 상태도 초기화
        }}
